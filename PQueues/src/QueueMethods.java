...
public class QueueMethods {
	...
	public static Queue<String> append(Queue<String> x, Queue<String> y) {
		Queue<String> tmp = new LinkedList<String>();
		Queue<String> cloneX = clone2(x);
		Queue<String> cloneY = clone2(y);

		while (!cloneX.isEmpty())
			tmp.offer(cloneX.poll());
		while (!cloneY.isEmpty())
			tmp.offer(cloneY.poll());
	...
	public static boolean isAlphabetical(Queue<String> x) {
		...
		while (iter.hasNext()) {
			...
			if (nextString.compareTo(previous) <= 0)
				return false;
			previous = nextString;
		}
		return true;
